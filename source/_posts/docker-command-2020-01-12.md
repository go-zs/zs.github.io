...
docekr exec -it <container-id> <command>  # 执行命令
docker inspect <container-id> # 查看容器信息
docker inspect nginx | grep -C 5 IPAddress
docker inspect -f "{{range .Mounts}} {{println .Source .Destination}} {{end}}" <container-id> # 看看挂载
docker inspect -f "{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}" <container-id> # 看看ip
docker inspect --format='{{.Name}} - {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $(docker ps -aq)  # 查看所有ip
docker system prune # 清理stopped container和无效的镜像，network
docker stats   # 查看资源
docker top <container-id> # 查看ps信息
docker cp helloworld:/test.txt ~/hello.txt
docker diff xxx  # 查看容器变化
docker rm $(docker stop $(docker ps -a -q)) # 删除所有容器
```

## docker build
```shell
docker build . -t xxx:tag
docker run --name xxx -d xxx:tag # 运行
docker commit xxx x:tag  ## 保存修改生成新image
docker push
docker pull
docker save  xxx > x.tar
docker load < x.tar
```
...
```shell
docker swarm init  # 初始化node
docker stack deploy -c docker-compose.yml <stack-name> # 创建stack
docker stack rm <stack-name> # 删除
docker service ls # 查看进程
docker service logs -f <service-id> # 查看日志
docker service update --force <service-id> # 强制更新
```